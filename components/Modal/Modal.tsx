...
const MODAL_OPTIONS = [{title: 'Intro', link: '/'}, {title: 'About', link: '/about'}, {title: 'Work', link: '/work'}, {title: 'Contact', link: '/contact'}];
const SOCIALS = [{title: 'Linkedin', link: 'https://www.linkedin.com'}, {title: 'Github', link: 'https://www.github.com/toyelowo'}, {title: 'Twitter', link: 'https://www.twitter.com'}];
...
export function Modal() {
  ...
  */
  ...
  return (
    <div className={`${styles.ModalWrapper} ${isModalOpen ? '' : styles.hideModal}`} ref={modalWrapperRef}>
      <div className={styles.Heading}>
        <h3 className={styles.Heading__title}><Link href='/'><a>Explore my portfolio</a></Link></h3>
      ...
      <div>
        <ul className={styles.NavWrapper}>
          {MODAL_OPTIONS.map((option) => (
            <li key={option.title} className={`${styles.Nav__List__item} ${getActiveClassName(option.link)}`}>
              <Link href={option.link}><a>{option.title}</a></Link>
            </li>
          ))}
      ...
      <div className={styles.Footer}>
        <ul className={styles.Socials}>
          {SOCIALS.map((social) => (
            <li key={social.title} className={styles.Socials__List_Item}><a href={social.link} rel="noreferrer" target="_blank">{social.title}</a></li>
          ))}
        </ul>
